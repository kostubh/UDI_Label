...
class InkscapeLabelGenerator:
    def __init__(self, root):
        ...
        self.svg_path = None
        # self.temp_svg = "temp_label.svg" # Removed for parallel safety
    ...
    def generate_labels(self, start, end, image_id, secondary_image_id):
        try:
            ...
            # Run in a separate thread so UI doesn't freeze while submitting tasks
            def run_parallel():
                try:
                    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
                        futures = []
                        for i in range(start, end + 1):
                            serial_number = f"{self.serial_prefix_var.get()}{i:04d}"
                            is_preview = (i == start)
                            future = executor.submit(
                                self.process_single_label, 
                                i, serial_number, image_id, secondary_image_id, is_preview
                            )
                        ...
                        for future in concurrent.futures.as_completed(futures):
                            ...
                            try:
                                ...
                                # Optional: store errors to show later

                    self.root.after(0, self.generation_complete)
                except Exception as e:
                    self.root.after(0, lambda msg=f"Error: {str(e)}": self.handle_error(msg))

            threading.Thread(target=run_parallel, daemon=True).start()

        except Exception as e:
            self.root.after(0, lambda msg=f"An error occurred: {str(e)}": self.handle_error(msg))
    ...
    def process_single_label(self, index, serial_number, image_id, secondary_image_id, is_preview):
        # Unique temp file for this thread
        import threading
        thread_id = threading.get_ident()
        temp_svg = f"temp_label_{serial_number}_{thread_id}.svg"
        ...
        try:
            ...
            if self.use_secondary_barcode.get() and secondary_image_id:
                sec_path = os.path.join(self.secondary_barcode_folder, f"{serial_number}.png")
            ...
            cmd.extend(["--export-filename", output_png])
            ...
            if is_preview:
                ...
                self.root.after(0, lambda: self.view_button.config(state=tk.NORMAL))
