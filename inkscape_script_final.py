...
class InkscapeLabelGenerator:
    ...
    def setup_ui(self):
        ...
        # Element IDs Frame
        elements_frame = tk.Frame(scrollable_frame, bg="#f0f0f0")
        elements_frame.pack(fill=tk.X, padx=20, pady=(0, 10))
        
        elements_label = tk.Label(elements_frame, text="SVG Element IDs:", bg="#f0f0f0", 
                                font=("Arial", 10, "bold"))
        elements_label.grid(row=0, column=0, sticky="w", pady=(0, 5))
        
        # Tspan IDs
        tspan_frame = tk.Frame(elements_frame, bg="#f0f0f0")
        tspan_frame.grid(row=1, column=0, sticky="w", pady=(0, 5))
        
        self.tspan_ids = []
        for i, default_id in enumerate(["tspan319", "tspan423"]):
            label = tk.Label(tspan_frame, text=f"Tspan {i+1}:", bg="#f0f0f0", 
                           font=("Arial", 10))
            label.grid(row=i, column=0, padx=(0, 5), sticky="w")
            
            entry = tk.Entry(tspan_frame, width=20, font=("Arial", 10))
            entry.insert(0, default_id)
            entry.grid(row=i, column=1, padx=(0, 10))
            self.tspan_ids.append(entry)
        
        # Image ID
        image_frame = tk.Frame(elements_frame, bg="#f0f0f0")
        image_frame.grid(row=2, column=0, sticky="w", pady=(5, 0))
        
        image_label = tk.Label(image_frame, text="Image ID:", bg="#f0f0f0", 
                             font=("Arial", 10))
    ...
    def save_config(self):
        ...
        self.config["tspan_ids"] = [entry.get() for entry in self.tspan_ids]
    ...
    def load_ui_from_config(self):
        ...
        if self.config.get("tspan_ids"):
            for entry, val in zip(self.tspan_ids, self.config["tspan_ids"]):
    ...
    def start_generation(self):
        ...
        try:
            ...
            # Get element IDs
            tspan_ids = [entry.get() for entry in self.tspan_ids]
            image_id = self.image_id.get()
            
            if not all(tspan_ids) or not image_id:
                messagebox.showerror("Error", "Please enter all element IDs")
                return
            ...
            threading.Thread(target=self.generate_labels, 
                           args=(start, end, tspan_ids, image_id), 
                           daemon=True).start()
    ...
    def generate_labels(self, start, end, tspan_ids, image_id):
        try:
            total = end - start + 1
            ...
            self.process_single_label(start, first_serial, tspan_ids, image_id, True)
            
            # Process remaining images
            for i in range(start + 1, end + 1):
                self.root.after(0, lambda curr=i-start+1: self.update_progress(curr, total))
                serial_number = f"{serial_prefix}{i:04d}"
                self.process_single_label(i, serial_number, tspan_ids, image_id, False)
    ...
    def process_single_label(self, index, serial_number, tspan_ids, image_id, is_preview):
        ...
        for elem in root.iter():
            if elem.tag.endswith("tspan") and elem.attrib.get("id") in tspan_ids:
