...
class InkscapeLabelGenerator:
    def __init__(self, root):
        ...
        self.output_svg_folder = "output_svgs"  # New SVG output folder
    ...
    def setup_ui(self):
        ...
        format_frame = tk.Frame(scrollable_frame, bg="#f0f0f0")
        format_frame.pack(fill=tk.X, padx=20, pady=(0, 10))
        
        format_label = tk.Label(format_frame, text="Output Formats:", bg="#f0f0f0", 
                              font=("Arial", 10, "bold"))
        format_label.pack(anchor="w", pady=(0, 5))
        
        self.output_png = tk.BooleanVar(value=True)
        self.output_svg = tk.BooleanVar(value=True)
        
        png_checkbox = tk.Checkbutton(format_frame, text="Generate PNG files", 
                                    variable=self.output_png, bg="#f0f0f0", 
                                    font=("Arial", 10))
        png_checkbox.pack(anchor="w")
        
        svg_checkbox = tk.Checkbutton(format_frame, text="Generate SVG files", 
                                    variable=self.output_svg, bg="#f0f0f0", 
                                    font=("Arial", 10))
        svg_checkbox.pack(anchor="w")
        
        # Background Option (only for PNG)
        ...
        transparent_checkbox = tk.Checkbutton(bg_option_frame, 
                                            text="PNG: Transparent Background (unchecked = white)", 
    ...
    def save_config(self):
        ...
        self.config["output_png"] = self.output_png.get()
        self.config["output_svg"] = self.output_svg.get()
        try:
    ...
    def load_ui_from_config(self):
        ...
        if "output_png" in self.config:
            self.output_png.set(self.config["output_png"])
        if "output_svg" in self.config:
            self.output_svg.set(self.config["output_svg"])
    ...
    def start_generation(self):
        ...
        if not self.output_png.get() and not self.output_svg.get():
            messagebox.showerror("Error", "Please select at least one output format (PNG or SVG).")
            return
        ...
        try:
            ...
            if self.output_svg.get():
                os.makedirs(self.output_svg_folder, exist_ok=True)
    ...
    def process_single_label(self, index, serial_number, image_id, secondary_image_id, is_preview):
        ...
        # Write modified SVG (always needed for PNG generation)
        tree.write(self.temp_svg, encoding='utf-8', xml_declaration=True)
        
        # Save SVG file if requested
        if self.output_svg.get():
            output_svg = os.path.join(self.output_svg_folder, f"label_{serial_number}.svg")
        ...
        if is_preview and self.output_png.get():
            self.preview_path = os.path.join(self.output_folder, f"label_{serial_number}.png")
            self.view_button.config(state=tk.NORMAL)
        elif is_preview and self.output_svg.get():
            self.preview_path = os.path.join(self.output_svg_folder, f"label_{serial_number}.svg")
            self.view_button.config(state=tk.NORMAL)
    ...
    def generation_complete(self):
        ...
        # Create success message with output info
        output_info = []
        if self.output_png.get():
            output_info.append(f"PNG files in '{self.output_folder}' folder")
        if self.output_svg.get():
            output_info.append(f"SVG files in '{self.output_svg_folder}' folder")
        
        success_message = "Labels generated successfully!\n\nOutput: " + " and ".join(output_info)
